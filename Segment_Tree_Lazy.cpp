...
void propagate(int pos, int left, int right) {
	int mid = (left + right) >> 1;
	node[pos * 2] += lazy[pos] * (mid - left + 1);
	node[pos * 2 + 1] += lazy[pos] * (right - mid);
	lazy[pos * 2] += lazy[pos];
...
void update(int pos, int left, int right, int x, int y, ll val) {
	...
	if (lazy[pos]) propagate(pos, left, right);
	int mid = (left + right) >> 1;
...
ll query(int pos, int left, int right, int x, int y) {
	...
	if (lazy[pos]) propagate(pos, left, right);
	int mid = (left + right) >> 1;
	return query(pos * 2, left, mid, x, y) + query(pos * 2 + 1, mid + 1, right, x, y);
}
